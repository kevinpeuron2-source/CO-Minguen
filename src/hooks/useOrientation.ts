...
export const useOrientation = () => {
  ...
  // 1. Lecture : Écouter les changements depuis Firebase (si connecté)
  useEffect(() => {
    if (!db) return; // Mode hors ligne

    // On écoute le document "session_actuelle" dans la collection "courses"
    const unsub = onSnapshot(doc(db, "courses", "session_actuelle"), (docSnap) => {
      if (docSnap.exists()) {
        const data = docSnap.data();
        // Mise à jour de l'état local si les données distantes changent
        if (data.balises) setBalises(data.balises);
        if (data.students) setStudents(data.students);
        if (data.groups) setGroups(data.groups);
        if (data.savedClasses) setSavedClasses(data.savedClasses);
      }
    }, (error) => {
  ...
  // 2. Écriture : Sauvegarder localement ET sur Firebase à chaque changement
  useEffect(() => {
    // LocalStorage (Backup toujours actif)
    ...
    if (db) {
      // On utilise un timeout pour "debouncer" (éviter d'écrire 100 fois par seconde)
      const timeoutId = setTimeout(() => {
        setDoc(doc(db, "courses", "session_actuelle"), {
          balises,
          students,
          groups,
          savedClasses,
          lastUpdate: Date.now()
        }, { merge: true }).catch(err => console.error("Erreur écriture Firebase:", err));
      }, 500);

      return () => clearTimeout(timeoutId);
    }
  }, [balises, students, groups, savedClasses]);
